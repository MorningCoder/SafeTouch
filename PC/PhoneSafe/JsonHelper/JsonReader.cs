...
namespace LitJson
{
    ...
    public class JsonReader
    {
        ...
        private IList<int>    automaton_stack;
        ...
        private JsonReader (TextReader reader, bool owned)
        {
            ...
            read_started = false;
            automaton_stack = new List<int> ();
            automaton_stack.Add ((int) ParserToken.End);
            automaton_stack.Add ((int) ParserToken.Text);
        ...
        public bool Read ()
        {
            ...
            while (true) {
                if (parser_return) {
                    if (automaton_stack[automaton_stack.Count - 1] == (int) ParserToken.End)
                        end_of_json = true;

                    return true;
                }

                current_symbol = automaton_stack[automaton_stack.Count - 1];
                automaton_stack.RemoveAt(automaton_stack.Count - 1);
                ...
                for (int i = entry_symbols.Length - 1; i >= 0; i--)
                    automaton_stack.Add (entry_symbols[i]);
            }
        }
