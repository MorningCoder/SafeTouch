									    ...
									    $error_msg['result']='-1';
										    $error_msg['message']="选择数据库失败！原因：" . mysql_error($link);
											    echo json_encode($error_msg);
													...
													$sql = "select * from Contacter where username = '$username'";
													$arr = array();
													$index = 0;
																	    	...
																	    	echo json_encode($error_msg);
																			    	exit();
																							...
																							while ($row = mysql_fetch_assoc($res)) 
																								{
																										$i = $index++;
																												$arr[$i]['contacter_name'] = $row['contacter_name'];
																														$arr[$i]['contacter_num'] = $row['contacter_num'];
																																$arr[$i]['contacter_location'] = $row['contacter_location'];
																																	}
